...
EcoTrack is a **local community sharing platform** that allows neighbors to share and lend tools, equipment, and other items. The backend is a Spring Boot microservices system demonstrating service discovery, API routing, inter-service communication with resilience patterns, Redis caching, AOP-based observability, and image management — all designed to run on a single local machine without cloud dependencies.
...
The platform currently consists of **5 microservices** (4 audited here, 1 additional):

```
React Frontend (Port 5173)
        │ HTTP / REST  (all traffic through gateway)
        ▼
┌───────────────────────────────┐
...
│          │  │          │   │                  │
│Netflix   │  │• CRUD    │   │• Registration    │
│Eureka    │  │• Search  │   │• Login (BCrypt)  │
│(no self- │  │• Paging  │   │• Profile CRUD    │
│register) │  │• Images  │   │• Password hashing│
...
| CSRF | Disabled | In user-service `SecurityConfig` |
...
├── communication/           # (5th service — not in audit scope)
├── frontend/                # React/Vite frontend
├── database/                # DB scripts/config (if any)
...
- JDK 17+
- Maven 3.8+
- MySQL 8.0+ (running, `db_items` and `db_users` databases created)
